...
echo "argument Fibonacci Fibonacci-memo" >> comparaison.dat
...
for entree in `seq 25 40`; do

  # ici $entree designe la valeur de la variable entree
  echo "je suis en train de traiter l'entree" $entree

# on envoie la valeur de entree a la commande /usr/bin/time ....
# cette commande execute fibo.native, recupere le temps utilisateur (-f'%U'),
# et stocke cette valeur dans le fichier /tmp/toto.txt
  echo $entree | /usr/bin/time -f'%U' -o /tmp/toto.txt ./fibo.native 
# on recupere ensuite dans TEMPS1 le contenu de /tmp/toto.txt
# notez a nouveau l'usage des `..`
  TEMPS1=`cat /tmp/toto.txt`

# meme chose avec fibo_memo.native
  echo $entree | /usr/bin/time -f'%U' -o /tmp/toto.txt ./fibo_memo.native
